...
#for idx in [100,118,119,162,163,195,212,2223,232,498]:
    ...
    origin_image_scene = vis.vis_object_detection(image_scene, test_set, obj_cls[:,0], obj_boxes, obj_scores[:,0])
    winname_scene = 'origin_image_scene'
    cv2.namedWindow(winname_scene)  # Create a named window
    cv2.moveWindow(winname_scene, 10, 10)
    cv2.imshow(winname_scene, origin_image_scene)
    cv2.imwrite(osp.join(args.vis_result_path, str(idx) + '.jpg'), origin_image_scene)

    # updated image_scene
    updated_image_scene = scene_graph.vis_scene_graph(image_scene, idx, test_set,
    ...
    updated_winname_scene = 'updated_image_scene'
    cv2.namedWindow(updated_winname_scene)  # Create a named window
    cv2.moveWindow(updated_winname_scene, 700, 10)
    cv2.imshow(updated_winname_scene, updated_image_scene)
    cv2.imwrite(osp.join(args.vis_result_path, str(idx) + '_updated' + '.jpg'), updated_image_scene)
