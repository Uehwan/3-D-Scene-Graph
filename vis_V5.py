...
def compare_position(curr_mean, prev_mean, prev_var, prev_pt_num, new_pt_num):
    I_x, I_y, I_z = check_distance(curr_mean, prev_mean, prev_var) 
    score = (I_x * I_y * I_z )
    score = float(score)
    return score
...
def check_distance(x, mean, var,beta=0.5):
...
def node_update(window_3d_pts, global_node, pose_3d_world_coord, curr_cls, cls_score, curr_color_hist,test_set ):
    try:
        new_pt_num = len(window_3d_pts)
        global_node_num = len(global_node)
        score = []
        cls_score = cls_score[0]
        w1, w2, w3 = 0.2/20.0, 13.0/20.0, 5.0/20.0
        for i in range(global_node_num):
            ...
            pos_sc = compare_position(pose_3d_world_coord, prev_mean, prev_var, prev_pt_num, new_pt_num)
        ...
        print("node_score {score:3.2f}".format(score=node_score))
...
def Draw_connected_scene_graph(node_feature, relation, img_count, test_set, sg, idx,cnt_thres=2,view=True,save_path='./vis_result/'):
    ...
    sg.render(osp.join(save_path,'scene_graph'+str(idx)), view=view, cleanup=True)
...
def make_window_size(width, height, obj_boxes):
    if( width<30):
        range_x_min = int(obj_boxes[0]) 
        range_x_max = int(obj_boxes[0]) + int(width)
    elif(width < 70):
        range_x_min = int(obj_boxes[0]) + int(width*1./4.)
        range_x_max = int(obj_boxes[0]) + int(width*3./4.)
    else:
        range_x_min = int(obj_boxes[0]) + int(width*2./5.)
        range_x_max = int(obj_boxes[0]) + int(width*3./5.)
        
    if (height < 30):
        range_y_min = int(obj_boxes[1]) 
        range_y_max = int(obj_boxes[1]) + int(height)
    elif (height < 70):
        range_y_min = int(obj_boxes[1]) + int(height*1./4.)
        range_y_max = int(obj_boxes[1]) + int(height*3./4.)
    else:
        range_y_min = int(obj_boxes[1]) + int(height*2./5.)
        range_y_max = int(obj_boxes[1]) + int(height*3./5.)
        ...
        for i, obj_ind in enumerate(obj_inds):  # loop for bounding boxes on each images
            ...
            if dataset == 'scannet':
                ...
                if(self.img_count ==0):
                    ...
                    node_score, max_score_index = node_update(window_3d_pts, self.data, curr_mean,
